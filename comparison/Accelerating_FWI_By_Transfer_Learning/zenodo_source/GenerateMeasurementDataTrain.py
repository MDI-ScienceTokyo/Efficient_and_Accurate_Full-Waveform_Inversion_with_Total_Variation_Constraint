...
# Set the number of elliptical holes.
n_dmg = 1
...
for case in range(numberOfCases):
    ...
    Um = forwardSolver_.forwardNSteps(
        u0_.clone(), u1_.clone(), gamma_, F_, Nx_, Ny_, N_, numberOfSources, device)[:, selx_, sely_,
         1:]  # zeroth timestep removed
    ...
    _, gradient = AdjointMethod.getAdjointGradient(
        forwardSolver, u0.clone(), u1.clone(), c, rho, gamma * 0 + 1, F,
        Nx, dx, Ny, dy, N, dt,
        numberOfSources, Um, selx, sely,
        device)
